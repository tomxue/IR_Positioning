...
// GPIO_146siy/GPT11_PWMEVT: IR_positioning SI_1V8 pin, for Y axis
// GPIO_139sw: IR_positioning Analog Switch IN pin
// GPIO_138six: IR_positioning SI_1V8 pin, for X axis
// McSPI4 applied
...
#define GPIO_146siy_OFFSET 		0x178   // P2437, low 16 bits
#define GPIO_138six_OFFSET 		0x168   // P2437, low 16 bits
...
#define GPIO146siy  0x00040000   		// Bit 21, GPIO149; Bit 16, GPIO144oe; Bit 10, GPIO138
#define GPIO138six  0x00000400   		// Bit 21, GPIO149; Bit 16, GPIO144oe; Bit 10, GPIO138
...
int DAQStart(char *argv)
{
    ...
    // GPIO146siy
    padconf = INT(map_base+GPIO_146siy_OFFSET);
    padconf &= 0xFFFF0000; //[15:0]=GPIO_146siy  - Clear register bits [15:0]
    padconf |= 0x00000004; //[15:0]=GPIO_146siy  - Select mux mode 4 for gpio
    INT(map_base+GPIO_146siy_OFFSET) = padconf;
    printf("GPIO_146siy_OFFSET - The register value is set to: 0x%x = 0d%u\n", padconf,padconf);
     // GPIO138six
    padconf = INT(map_base+GPIO_138six_OFFSET);
    padconf &= 0xFFFF0000; //[15:0]=GPIO_138six  - Clear register bits [15:0]
    padconf |= 0x00000004; //[15:0]=GPIO_138six  - Select mux mode 4 for gpio
    INT(map_base+GPIO_138six_OFFSET) = padconf;
    printf("GPIO_138six_OFFSET - The register value is set to: 0x%x = 0d%u\n", padconf,padconf);
    ...
    padconf &= ~(GPIO139sw+GPIO143cs+GPIO144oe+GPIO145clk+GPIO146siy+GPIO138six);  // Set GPIO_139sw, GPIO143cs, GPIO144oe, GPIO_145clk, GPIO_146siy and GPIO_138six to output
    ...
    {
        // sw, clk, si
        padconf &= ~(GPIO145clk+GPIO146siy+GPIO138six);    // set GPIO145clk, GPIO146siy and GPIO138six low
        INT(map_base+GPIO5_DATAOUT_OFFSET) = padconf;
        ...
        if(CLKCount == 1)
        {
            if(sampleXorY == 0)
            {
                padconf |=  GPIO146siy;    // Set GPIO_146siy high
                INT(map_base+GPIO5_DATAOUT_OFFSET) = padconf;
            ...
            {
                padconf |=  GPIO138six;    // Set GPIO_138six high
                INT(map_base+GPIO5_DATAOUT_OFFSET) = padconf;
        ...
        if(CLKCount == XYLoop)
        {
            ...
            for(delayCount;delayCount<20;delayCount++)
                INT(map_base+GPIO5_DATAOUT_OFFSET) = padconf;
            // cs
